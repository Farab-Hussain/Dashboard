...
function get_bidder_stats() {
    ...
    try {
        ...
        // Get total count
        $stmt = $pdo->query("SELECT COUNT(*) as total FROM bidders");
        $total = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        error_log("Total bidders: " . $total);
        
        // Get active bidders (using the correct 'status' column)
        $stmt = $pdo->query("SELECT COUNT(*) as active FROM bidders WHERE status = 'active'");
        $active = $stmt->fetch(PDO::FETCH_ASSOC)['active'];
        error_log("Active bidders: " . $active);
        
        // Get inactive bidders
        $stmt = $pdo->query("SELECT COUNT(*) as inactive FROM bidders WHERE status = 'inactive'");
