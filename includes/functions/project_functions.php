...
function ensure_quotes_column_exists() {
    global $pdo;
    
    try {
        // Check if quotes column exists
        $stmt = $pdo->query("SHOW COLUMNS FROM projects LIKE 'quotes'");
        if ($stmt->rowCount() == 0) {
            // Add quotes column
            $pdo->exec("ALTER TABLE projects ADD COLUMN quotes TEXT NULL");
        }
    } catch (PDOException $e) {
        error_log("Error ensuring quotes column exists: " . $e->getMessage());
    }
}
...
function create_project($data, $files) {
    ...
    try {
        ...
        $stmt->execute([
            ...
            $data['reason_other'],
            $data['completed'],
            isset($data['quotes']) ? $data['quotes'] : null,
            $_SESSION['user_id']
        ]);
...
function get_project_stats() {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) as active FROM projects WHERE due_date >= CURDATE()");
        $stmt->execute();
        $active = $stmt->fetch(PDO::FETCH_ASSOC)['active'];
        
        // Get completed projects (based on completed field)
        $stmt = $pdo->prepare("SELECT COUNT(*) as completed FROM projects WHERE completed = 1");
        $stmt->execute();
