...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
        ...
        case 'update_project':
            if (isset($_POST['project_id'])) {
                // Due date
                $due_year = $_POST['due_year'] ?? date('Y');
                $due_month = $_POST['due_month'] ?? date('m');
                $due_day = $_POST['due_day'] ?? date('d');
                $due_date = sprintf('%04d-%02d-%02d', $due_year, $due_month, $due_day);
                
                // Due time
                $hour = isset($_POST['due_hour']) && $_POST['due_hour'] !== '' ? (int)$_POST['due_hour'] : 0;
                $minute = isset($_POST['due_minute']) && $_POST['due_minute'] !== '' ? (int)$_POST['due_minute'] : 0;
                $ampm = $_POST['due_ampm'] ?? 'AM';
                if ($ampm === 'PM' && $hour < 12) $hour += 12;
                if ($ampm === 'AM' && $hour === 12) $hour = 0;
                $due_time = sprintf('%02d:%02d:00', $hour, $minute);
                $time_zone = $_POST['time_zone'] ?? 'EST';
                
                // Assign date
                $assign_year = $_POST['assign_year'] ?? date('Y');
                $assign_month = $_POST['assign_month'] ?? date('m');
                $assign_day = $_POST['assign_day'] ?? date('d');
                $assign_date = sprintf('%04d-%02d-%02d', $assign_year, $assign_month, $assign_day);
                ...
                $status_no_result = 0;
                $status = $_POST['status'] ?? 'not_submitted';
                switch ($status) {
                ...
                // Quotes field (admin only)
                $quotes = isset($_POST['quotes']) ? trim($_POST['quotes']) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
                ...
                if (isset($_POST['user_id'], $_POST['new_password'], $_POST['confirm_password'])) {
                    if ($_POST['new_password'] !== $_POST['confirm_password']) {
                        ...
                        if ($result['success']) {
                            ...
                            $_SESSION['error_message'] = $result['error'] ?? "Error changing password.";
                        }
                    }
                }
                break;
            ...
            case 'update_quotes':
                if (isset($_POST['project_id']) && isset($_POST['quotes'])) {
                    $project_id = $_POST['project_id'];
                    $quotes = trim($_POST['quotes']);
                    
                    try {
                        $stmt = $pdo->prepare("UPDATE projects SET quotes = ? WHERE id = ?");
                        $result = $stmt->execute([$quotes, $project_id]);
                        
                        // Check if this is an AJAX request
                        if (isset($_POST['ajax']) || (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest')) {
                            header('Content-Type: application/json');
                            if ($result) {
                                echo json_encode(['success' => true, 'message' => 'Quotes updated successfully']);
                            } else {
                                echo json_encode(['success' => false, 'message' => 'Error updating quotes']);
                            }
                            exit();
                        } else {
                            if ($result) {
                                $_SESSION['success_message'] = "Quotes updated successfully!";
                            } else {
                                $_SESSION['error_message'] = "Error updating quotes.";
                            }
                        }
...
<div class="dashboard-container">
    ...
    <div class="dashboard-header">
        <div class="tabs">
            ...
            <div class="tab-navigation">
                ...
                <a href="?tab=users" class="tab-btn <?= $active_tab === 'users' ? 'active' : '' ?>">
                ...
                <div class="table-header">
                    ...
                    <table class="projects-table">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="quotes-content">
                                            <?php if (!empty($project['quotes'])): ?>
                                                <span class="quotes-text" title="<?= htmlspecialchars($project['quotes']) ?>">
                                                    <?= htmlspecialchars(substr($project['quotes'], 0, 50)) ?><?= strlen($project['quotes']) > 50 ? '...' : '' ?>
                                                </span>
                                                <button class="btn-icon" onclick="viewQuotes(<?= $project['id'] ?>)" title="View Full Quote">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                            <?php else: ?>
                                                <span class="no-quotes">No quotes</span>
                                                <button class="btn-icon" onclick="addQuotes(<?= $project['id'] ?>)" title="Add Quote">
                            ...
                            ?>
                                <tr>
                                    <td colspan="<?= $_SESSION['role'] === 'admin' ? '10' : '9' ?>" class="no-data">
                                        <i class="fas fa-project-diagram"></i>
                            ...
                            ?>
                                <tr>
                                    <td colspan="<?= $_SESSION['role'] === 'admin' ? '10' : '9' ?>" class="error">
    ...
    <div class="tab-content <?= $active_tab === 'users' ? 'active' : '' ?>" id="users-tab">
        <div class="dashboard-section">
            <div class="section-header">
                <h2><i class="fas fa-users"></i> Users Management</h2>
                <button type="button" class="btn-primary" onclick="showModal('add-user-modal')">
            ...
            <div class="stats-cards">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <h3>Operators</h3>
                        <p><?= $system_stats['users']['operator'] ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="table-card">
                <div class="table-header">
                    ...
                    <div class="table-actions">
                        ...
                        <button type="button" class="btn-secondary" onclick="clearUserFilters()" title="Clear Filters">
                            <i class="fas fa-times"></i> Clear
                        </button>
                    </div>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($user['id'] == $_SESSION['user_id']): ?>
                                            <span class="role-badge you">
                                                <i class="fas fa-user-shield"></i>
                                                <?= ucfirst(htmlspecialchars($user['role'])) ?> (You)
                                            </span>
                                        <?php else: ?>
                                            <select class="role-select" onchange="changeUserRole(<?= $user['id'] ?>, this.value)" title="Change Role">
                                    ...
                                    <td>
                                        <span class="status-badge <?= isset($user['is_active']) && $user['is_active'] ? 'active' : 'inactive' ?>">
                                            <i class="fas fa-circle"></i>
        ...
        <div class="modal-content">
            ...
            <form id="add-project-form" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-section">
                    <h3>Quotes</h3>
                    <div class="form-group">
                        <label for="quotes">Project Quotes</label>
                        <textarea id="quotes" name="quotes" rows="4" placeholder="Enter project quotes or notes here..."></textarea>
                    </div>
                </div>
                <?php endif; ?>
        ...
        <div class="modal-content">
            ...
            <form id="edit-project-form" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-section">
                    <h3>Quotes</h3>
                    <div class="form-group">
                        <label for="edit_quotes">Project Quotes</label>
                        <textarea id="edit_quotes" name="quotes" rows="4" placeholder="Enter project quotes or notes here..."></textarea>
                    </div>
                </div>
                <?php endif; ?>
